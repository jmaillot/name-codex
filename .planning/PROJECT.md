...
## Shipped Milestone: v1.5 Naming Convention Expansion (2026-08-25)
...
(None — all v1.5 requirements shipped. Define next milestone via `/gsd-new-milestone`.)
...
- v1.4 shipped on 2026-08-24: 2 phases (11–12), 5 plans, 8 tasks, 31 files (+845/−2023), src/ 5,057 LOC TS/TSX/CSS — full light token palette + theme-scoped WCAG gate, boot-time theme resolution (`name-codex-theme-v14-0` key; deliberately outside the legacy `-v11-6` key family), sun/moon CommandBar toggle with 175ms fade, dual-theme smoke matrix user-approved 18/18. Annotated tag `v1.4`.
- v1.5 shipped on 2026-08-25: 3 phases (13–15), 13 plans, 25 tasks, 60 commits, 68 files (+3,663/−598) — catalog 25→45 conventions across 7→11 categories as data-driven JSON; EN/FR locale keys grew to full parity on every new convention/field/tip; test suite 137→183 with catalog-integrity proofs (45 conventions / 11 categories / search reachability ×20 / EN=FR); verification COMPLETE-WITH-WAIVER (manual smoke matrix waived by user approval). Annotated tag `v1.5`.
...
<details>
<summary>Shipped: v1.3 — Developer Portal UI (2026-08-24)</summary>
...
<details>
<summary>Shipped: v1.4 — Light Mode (2026-08-24)</summary>

- Light palette: all 28 UI-SPEC contract value groups populated into `:root[data-theme="light"]` (+`color-scheme: light`); navy-tinted surfaces #e4e9f2 canvas → #ffffff panels, darkened accent #2e6fe0/#1e56c0, tokenized brand-title gradient; zero changes outside tokens.css; worst AA pair --text-secondary on canvas = 4.92:1 (Phase 11).
- Theme-scoped gate: `check:tokens` parses `:root` and `:root[data-theme="light"]` separately (never merged), proves 9-pair WCAG matrix per theme (18/18) with var()-chain resolution and negative-control testing (Phase 11).
- Boot-time theme resolution: inline pre-paint script + `src/lib/theme.ts`; allowlist-validated localStorage key `name-codex-theme-v14-0`; absent key = follow OS preference; no flash of wrong theme; no live matchMedia listener by design (D-05/D-08) (Phase 12).
- Sun/moon toggle pill: `[theme][EN/FR]` CommandBar cluster, target-theme lucide icons, bilingual tooltip/aria-labels parity 374=374 (`ui.themeSwitchToDark/Light`), 175ms ease-out color-only fade via transient `html.theme-anim` class with reduced-motion guard and unmount cleanup (Phase 12).
- Verification: mechanical gates green (137 tests zero added, build/lint, check:tokens 18/18), PERF-01..03 grep re-proofs intact, dual-theme smoke matrix 9×dark/light user-run 18/18 PASS (THEME-L-04); code review 0 critical, WR-01 fixed (`a3567b1`) + IN-01..07 resolved, IN-06 skipped per locked D-05.
- Milestone: 2 phases (11–12), 5 plans, 8 tasks, 31 files (+845/−2023), src/ 5,057 LOC TS/TSX/CSS, same-day execution (~3h).

</details>

**Shipped: v1.5 — Naming Convention Expansion (2026-08-25)**

- New categories: Microsoft Teams (PRJ-/DEPT-/SRV- teams + Entra naming-policy format), SharePoint (Team-site/Communication-site/Hub-site), Power Platform (environment per ALM strategy), Purview (sensitivity label + sublabel, retention label/policy, DLP policy) — brand SVG icons wired into nav with theme-aware tokens (Phase 13).
- Expansions: Azure CAF storage account (no-hyphen lowercase outlier via generic per-field `allowedPattern` + 24-char uniqueness limit), key vault, NSG, public IP; Intune Autopilot device name (`%SERIAL%`/`%RAND:x%` macros within hard 15-char budget), Group Tag, Update Ring, Assignment Group (Phase 14).
- Builder correctness sweep: pure `assembleRawName` from live `builderSegments` (custom values no longer silently dropped), no-op Prefix segments replaced with literal prefixes at data level, `validateName` synced to `activeFields`, stale-select sentinel option, phantom-default numeric input removed (WR-01..WR-06).
- Verification: 9/9 mechanical gates green with verbatim evidence; vitest suite 137→183 incl. 4 new catalog-integrity proofs (45 conventions, 11 categories, search reachability ×20, EN=FR parity); v1.4..HEAD zero-code-diff proof with each of 36 survivors justified to its originating commit; dual smoke matrix waived by direct user approval — verdict COMPLETE-WITH-WAIVER (15-VERIFICATION.md).
- Milestone: 3 phases (13–15), 13 plans, 25 tasks, 60 commits, 68 files (+3,663/−598), 2 days execution.

## Next Milestone

Not yet defined. Start with `/gsd-new-milestone` (questioning → research → requirements → roadmap). Candidates carried in Future Candidates below: DATA-01/02 JSON schema validation, LOCL-01 extra languages, additional CAF resources (ACR, AKS, Cosmos DB…). Code-review follow-up carried from v1.5: truncation-before-validate ordering blocker in `useAppState.ts` (see 15-REVIEW.md).
...
---
*Last updated: 2026-08-25 after v1.5 milestone*
