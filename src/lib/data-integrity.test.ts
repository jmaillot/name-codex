...
describe("data integrity", () => {
  ...
  it("catalog contains exactly 60 conventions (25 existing + 19 new + 6 CAF -1 grouped VNet GW + 4 data-tier + 3 observability + 3 network +1 LB split)", () => {
    expect(allConventions.length).toBe(60)
  })
  ...
  const NEW_CONVENTION_IDS = [
    ...
    "azure-load-balancer-internal", "azure-load-balancer-external", "azure-application-gateway", "azure-firewall",
  ]

  it("every new convention is reachable via global-search (name/description/id/pattern match)", () => {
    expect(NEW_CONVENTION_IDS.length).toBe(36)
